...
import { saveWaitlistSubmission } from "../lib/supabaseClient";
...
export default function QuestionnaireForm({
  ...
  // Parse price to see if it is 300 or above
  const parsePrice = (text: string): number => {
    if (!text) return 0;
    // Convert Arabic numerals to English numerals
    const arabicNorm = text.replace(/[٠-٩]/g, (d) => "٠١٢٣٤٥٦٧٨٩".indexOf(d).toString());
    // Extract digits
    const match = arabicNorm.match(/\d+/);
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    setTimeout(async () => {
      ...
      const priceValue = parsePrice(answers.predictedPrice);
      const isAutoApproved = priceValue >= 300;
      const submissionStatus = isAutoApproved ? 'approved' : 'pending';
      ...
      // Save registration status and details in browser storage
      localStorage.setItem("user_waitlist_registered", "true");
      localStorage.setItem("user_waitlist_details", JSON.stringify(submission));
      if (isAutoApproved) {
        localStorage.setItem("user_waitlist_approved", "true");
      } else {
        localStorage.setItem("user_waitlist_approved", "false");
      }
      ...
      try {
        await saveWaitlistSubmission({
          id: submission.id,
          hair_type: submission.hairType || "",
          takes_care: submission.takesCare || "",
          is_brush_important: submission.isBrushImportant || "",
          predicted_price: submission.predictedPrice || "",
          phone_number: submission.phoneNumber,
          email: submission.email,
          queue_number: submission.queueNumber,
          submitted_at: submission.submittedAt,
          status: submissionStatus
        });
      } catch (errWaitlist) {
        console.warn("Could not save waitlist to Supabase directly:", errWaitlist);
      }
      ...
      // If auto-approved, redirect immediately (Unless they are the logged in admin)
      const isAdminLoggedIn = !!localStorage.getItem("smoozice_user");
      if (isAutoApproved && !isAdminLoggedIn) {
        window.location.href = "https://smoozieebr.duckdns.org/";
      }
    }, 1200);
  };
